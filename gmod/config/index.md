# config
Read and write cheat settings from Lua using string keys; each key has a fixed type (`bool`, `int`, `float`, `color`, `multi`, or `string`). The in-game menu and [ui](/gmod/ui/) controls reference the same keys-often via `config_name` on a widget.
...
- Writes a value; `value` must match the key’s type:
  ...
  - **int** / **float** — number
  - **string** — string
  - **color** — table with **1-based** indices `1-4`, channels **0–255** (stored internally normalized)
